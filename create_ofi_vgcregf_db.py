...
fact_player_tour.to_sql("fact_run", con=engine, if_exists='replace', index=False)
mydb.commit()

#%%
mycursor = mydb.cursor()

mycursor.execute("CREATE TABLE team_ots (ID INT, Pokemon VARCHAR(255), Objeto VARCHAR(255),  Habilidad VARCHAR(255), Teratipo VARCHAR(255), Mov1 VARCHAR(255),Mov2 VARCHAR(255), Mov3 VARCHAR(255), Mov4 VARCHAR(255))")

team_list.to_sql('team_ots', con=engine, if_exists='replace', index=False)
...
mycursor.execute("CREATE TABLE rounds (ID INT, Ronda VARCHAR(255), Rival VARCHAR(255),  Resultado VARCHAR(255))")

round_list.to_sql('rounds', con=engine, if_exists='replace', index=False)
